...
class DroneTrackingService:
    ...
    def _get_vehicle_id_from_serial(self, serial: str) -> Optional[str]:
        ...
        for vehicle in vehicles:
            # FIX: Use correct field name from Skydio API response
            vehicle_serial = vehicle.get("vehicle_serial") or vehicle.get("serial_number") or vehicle.get("serial")
            self.logger.debug(f"Checking vehicle: {vehicle_serial} vs {serial}")
            self.logger.debug(f"Vehicle data: {vehicle}")
            
            if vehicle_serial == serial:
                self.logger.debug(f"Found matching vehicle! Looking for ID...")
                # FIX: For Skydio API, use the vehicle_serial as the ID
                vehicle_id = vehicle.get("id") or vehicle.get("uuid") or vehicle.get("vehicle_id") or vehicle_serial
                self.logger.debug(f"Vehicle ID found: {vehicle_id}")
                ...
                if vehicle_id:
                    ...
                    self.logger.error(f"Vehicle {serial} found but no ID field! Available keys: {vehicle.keys()}")
                    break
    ...
    def _get_vehicle_position(self, serial: str) -> Optional[Tuple[float, float, Optional[float]]]:
        """Get current position for a specific vehicle by serial number - FIXED FOR SKYDIO API"""
        ...
        # FIX: Skydio API pattern - check if vehicle is currently flying
        vehicles_data = self._make_skydio_request("v0/vehicles")
        if not vehicles_data:
            return None
        ...
        flight_status = current_vehicle.get("flight_status")
        is_online = current_vehicle.get("is_online", False)
        
        self.logger.debug(f"Vehicle {serial} status: flight_status={flight_status}, is_online={is_online}")
        
        if flight_status != "FLYING" and not is_online:
            self.logger.info(f"Vehicle {serial} is not currently flying (status: {flight_status})")
            return None
        
        # FIX: For currently flying vehicles, try to get the most recent flight data
        # since Skydio API doesn't provide true "live" telemetry via REST
        flights_data = self._make_skydio_request(f"v0/flights?vehicle_serial={serial}&limit=1")
        
        if not flights_data or not flights_data.get("flights"):
            self.logger.warning(f"No recent flight data for {serial}")
            return None
            
        latest_flight = flights_data["flights"][0]
        flight_id = latest_flight.get("flight_id")
        
        # Check if this flight is recent (within last hour for active tracking)
        takeoff_time = latest_flight.get("takeoff")
        landing_time = latest_flight.get("landing")
        
        # If flight has landed, it's not current
        if landing_time:
            self.logger.info(f"Latest flight for {serial} has already landed")
            return None
            
        if not flight_id:
            self.logger.error(f"No flight ID found for latest flight of {serial}")
            return None
            
        # Get telemetry from the current/recent flight
        # FIX: Use correct telemetry endpoint pattern
        telemetry_endpoints = [
            f"v0/flights/{flight_id}/telemetry?limit=1&order=desc",  # Most recent point
            f"v0/flights/{flight_id}/telemetry/latest",
            f"v0/flights/{flight_id}/telemetry"
        ]
        
        for endpoint in telemetry_endpoints:
            self.logger.debug(f"Trying flight telemetry endpoint: {endpoint}")
            telemetry_data = self._make_skydio_request(endpoint)
            
            if telemetry_data:
                self.logger.debug(f"Got flight telemetry from {endpoint}: {list(telemetry_data.keys())}")
                position = self._extract_position(telemetry_data)
                if position:
                    self.logger.info(f"Got current position from active flight for {serial}")
                    return position
        
        # FIX: If no telemetry from flight, try takeoff position as last known
        takeoff_lat = latest_flight.get("takeoff_latitude") 
        takeoff_lon = latest_flight.get("takeoff_longitude")
        
        if takeoff_lat is not None and takeoff_lon is not None:
            self.logger.info(f"Using takeoff position for {serial} (no recent telemetry)")
            return (float(takeoff_lat), float(takeoff_lon), None)
        
        self.logger.warning(f"No position data available for vehicle {serial}")
        return None
