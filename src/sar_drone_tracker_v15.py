                        ...
                        try:
                            ...
                            if position:
                                ...
                                if success:
                                    ...
                                    print(f"📍 LIVE {device_id}: {latitude:.6f}, {longitude:.6f}{alt_str}")
                                    
                                    self.logger.debug(f"Live position update for {serial}: {latitude:.6f}, {longitude:.6f}")
                                else:
                                    self.logger.error(f"Failed to update CalTopo for {serial}")
                            else:
                                self.logger.debug(f"No GPS data in telemetry for {serial}")
