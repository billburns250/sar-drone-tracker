...
async def track_single_drone(tracker, serial):
    ...
    print(f"Starting tracking for Drone ID [{serial}] -> CalTopo Object [{device_id}]")
    ...
    if status:
        ...
        print(f"Found {serial}: {name} | Flight: {flight_status} | Battery: {battery_pct:.1f}% | Attachments: {attachments_str} | Online: {is_online}")
    else:
                    ...
                    try:
                        ...
                        if lat is not None and lon is not None:
                            ...
                            if not first_position_logged:
                                ...
                                print(f"Posting CalTopo Updates As: {example_url}")
...
async def main():
    ...
    try:
        ...
        print("SAR Drone Tracker")
        print(f"Tracking: {tracker.drone_serials}")
        print(f"Device IDs: {[tracker._generate_device_id(s) for s in tracker.drone_serials]}")
        print(f"Debug Mode: {'Enabled' if tracker.debug_enabled else 'Disabled'}")
        print("=" * 60)
